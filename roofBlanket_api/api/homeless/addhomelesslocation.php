...
// get search query
if(  isset($_GET["id"]) && isset($_GET["long"])  && isset($_GET["lat"]) && isset($_GET["notes"])  ) {
    $result = $homelessLocationDAO->add_location( $_GET["id"],$_GET["long"], $_GET["lat"], $_GET["notes"] );
    // $result = false;
...
if( $result ) {
    ...
    // tell the user no items found
    echo json_encode(
        array("Homeless Location" => "Homeless Location could not be added.")
    );
}
?>